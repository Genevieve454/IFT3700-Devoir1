...
relationship_ordering = {'Unmarried':0,'Wife':1,'Husband':2,'Not-in-family':3,'Own-child':4,'Other-relative':5}
...
def prep(person):
    ...
    if person[1] != "?":
        result = np.append(result, occupationClassif[person[1]] / 2)  # occupation, range from 0 to 2
    else:
        result = np.append(result, 0.5)

    result = np.append(result, maritalstatus_issingle[person[2]])
    result = np.append(result, relationship_ordering[person[3]]/5)
...
# Transforme le gender en 0 et 1
